...
[html.ru](http://htmlbook.ru/html/form)
...
Коли форма відправляється на сервер, управління даними передається програмі, заданої атрибутом `action` тега `<form>`. Попередньо браузер готує інформацію у вигляді пари `«ім'я = значення»`, де ім'я визначається атрибутом name тега `<input>`, а значення введено користувачем або встановлено в поле форми за замовчуванням. Якщо для відправки даних використовується метод GET, то адресний рядок може приймати наступний вигляд.
...
- [accept-charset](http://htmlbook.ru/html/form/accept-charset) Встановлює кодування, в якому сервер може приймати та обробляти дані.
- [action](http://htmlbook.ru/html/form/action) Адреса програми або документа, який обробляє дані форми.
- [autocomplete](http://htmlbook.ru/html/form/autocomplete) Включає автозаповнення полів форми.
- [enctype](http://htmlbook.ru/html/form/enctype) Спосіб кодування даних форми.
- [method](http://htmlbook.ru/html/form/method) Метод протоколу HTTP.
- [name](http://htmlbook.ru/html/form/name) Ім'я форми.
- [novalidate](http://htmlbook.ru/html/form/novalidate) Скасовує вбудовану перевірку даних форми на коректність введення.
- [target](http://htmlbook.ru/html/form/target) Ім'я вікна або фрейму, куди обробник буде завантажувати повертається результат.
...
[Формы, элементы управления](https://learn.javascript.ru/form-elements)
...
[Как сделать всплывающую форму](https://myrusakov.ru/js-popup-form.html)
