...
[яваскрипт.укр](http://яваскрипт.укр)
...
Тип `object` (обєкт) використовується для збереження колецій даних, або більш складних обєктів. 



#### symbol

Тип `symbol` (символ) використовується для створення унікальних ідентифікаторів обєктів. 
...
Для представлення значень у JavaScript використовуються літерали. Це  фіксовані значення, а не змінні, які ви буквально надаєте у вашому  скрипті. 
...
#### Додаткові коми в літералах масивів
...
https://developer.mozilla.org/uk/docs/Web/JavaScript/Reference/Lexical_grammar#Numeric_literals)

### Об'єктні літерали

Об'єктні літерали - це список з нуля або більше пар імен властивостей та асоційованих значень об'єкту, взятих у фігурні дужки (`{}`).
...
Далі наведено приклад об’єктного літералу. Перший елемент об'єкта `car` означується властивістю `myCar `та присвоюється йому нове текстове значення "`Saturn`"; другому елементу, властивості `getCar`, негайно присвоюється результат виклику функції `(carTypes("Honda"));` третьому елементу, властивості `special` , присвоюється значення існуючої змінної (`sales`).
...
Крім того, ви можете використовувати цілочсельний або  рядковий літерал для назви властивості або вкладати об'єкт всередину  іншого. У наступному прикладі використовуються ці можливості.
...
Імена властивостей об'єкта можуть бути будь-якими  рядками, включаючи порожні. Якщо ім'я властивості не було б дійсним  ідентифікатором чи числом JavaScript, воно повинно бути укладено в  лапки.

До імен властивостей, які не є дійсними ідентифікаторами, не можна отримати доступ до властивості через крапку (`.`), Але *можна* отримати доступ до них та встановити нотацію, схожу на масив ("`[]`").
...
#### Розширені об'єктні літерали

У ES2015 об'єктні літерали розширюються для підтримки налаштування прототипу при побудові, скорочення назначення для `foo: foo `, означення методів, здійснення викликів `super` та обчислення імен властивостей з виразами.

Разом вони також зближують об'єктні літерали та декларації класів (`class` ), і дозволяють розробці на основі об'єктів користуватися деякими тими ж зручностями.
...
##### `in` (існування властивості або індекс масиву)
