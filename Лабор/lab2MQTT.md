...
- [ ] зробіть публікацію
- [ ] відкрийте MQTT Explorer, знайдіть це повідомлення і передивіться його значення
...
- [ ] З розділу палітри `Input` вставте вузол `MQTT In`. В налаштуваннях Server добавте новий брокер MQTT з назвою `mosquitto` і адресою серверу http://test.mosquitto.org   
