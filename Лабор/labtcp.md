...
Отримайте дані відповідно до варіанту.
...
- [ ] Перейдіть в меню `Захоплення -> Опції`, виберіть інтерфейс `Loopback adapter`, який захоплює пакети що циркулюють між застосунками на одному ПК
...
- [ ] Релаізуйте в Node-RED фрагмент, який показаний на рис.3, але **вказавши номер порту відповідно до варіанту**
...
- [ ] Запустіть Node-RED і створіть новий проект.
...
- [ ] Модифікуйте фрагмент програми з рис.5 так, щоб відправлялися бінарні дані з вузла буфер, що показаний на рис.6. 
...
- [ ] Проконтролюйте налаштування серверу TCP в `Packet Sender` (рис.4).
- [ ] Використовуючи вузол `tcprequest` добавте в програму наступний фрагмент (рис.8)
...
- [ ] Використовуючи вузли `tcpin` та `tcpout` добавте в програму наступний фрагмент (рис.9). Зверніть увагу що там повинне бути вписане Ваше ім'я та налаштування порту.
...
- [ ] Використовуючи вікно налагодження проаналізуйте зміст `msg`, отриманого з вузла `TCP-IN` (див. приклад рис.9)
...
- [ ] Відкрийте сторінку в браузері https://zxing.org/w/decode.jspx і перевірте її роботу. Для цього завантажте будь-який файл з зображенням штрих-коду, наприклад [звідси](https://drive.google.com/open?id=1HqK3ItqtxigkonY8VWg2fHVsA_XaEwY2) (див. рис.11).

![рис.1](WEBAPIMedia/Рисунок1.png)                               

рис.11

Натисніть «Відправити запит»,  при вдалій обробці запиту буде повернений результат з кодом, подібно до наведеного на рис.12.   
...
- [ ] Вибиріть перший запис у списку запит і відкрийте панель «Заголовки».
...
- [ ] Налаштуйте його таким чином, щоб він зчитував дані з файлу з штрих-кодом та виводив його вміст на панель відлагодження (Debug) див.рис.14.Зверніть увагу, що виведення в `Debug` треба робити як `complete msg object` а не як `msg.payload`.  

![рис.6.](WEBAPIMedia/Рисунок6.png)  

рис.14.

- [ ] Зробіть розгортання потоку, та з використанням панелі відлагодження подивіться що зміст файлу виводиться як масив байт (рис.14). 
...
- [ ] Модифікуйте програму, як це показано на рис.15. 
...
- [ ] Модифікуйте програму, як це показано на рис.16. Зверніть увагу, що для кращого тестування бажано вивести як вхідне так і вихідне повідомлення `HTTP request`. 
...
- [ ] З розділу `parser` палітри вузлів вставте `HTML`. Модифікуйте програму, як це показано на рис.17. Селектор вибирає елемент HTML з ID=”result” у якого вибирає усі-елементи нащадки з тегом «pre».
...
- [ ] Добавте вузол-функцію для присвоєння значення `msg.ID` рівним номеру штрих-коду при позитивній відповіді, та `ERROR` при помилці обробки (рис.18).  
