...
На відміну від системи з проміжною компіляцією, мова JavaScript інтерпретована. Тобто аналізується та виконується вихідний код програми **JavaScript-рушієм**, без попередньої компіляції. Цей рушій може бути вбудований наприклад в браузер. Останні версії HTML (HTML5) включають в себе обов'язкову підтримку JavaScript. 
...
Змінна в JavaScript може містити будь-які дані. У один момент там може бути рядок, а в інший – число. Коли змінна не прив'язується до конкретного типу, але при цьому типи даних існують, це називається "динамічною типізацією".  У наведеному нижче коді, одній змінній присвоюють різні за типом значення. Оператор `typeof` повертає значення типу.
...
У JavaScript існує вісім типів даних: `number`, `BigInt`, `string`, `boolean`, `null`, `undefined`, `object`, `symbol`. Спочатку розглянемо базові (примітивні) типи, усі інші типи будуть розглянуті пізніше. Детальніший розгляд типів можна подивитися у [розділі "Типи даних та літерали" з довідника](../Довідники/js/js.md#types).

У JavaScript примітивні типи даних `number`, `BigInt`, `string`, `boolean`, подібно об'єктам мають методи і властивості, до яких можна звертатися через крапку. Для цього JavaScript при такому доступі створює спеціальний об'єкт-обгортку, який надає потрібну функційність, після чого видаляється. Ці об'єкти мають відповідні назви  `String`, `Number`, `Boolean` і `Symbol`, і мають різний набір методів. Наприклад, у наведеному нижче фрагменті, метод `toUpperCase` з об'єкту типу`String` виводить стрічку зі зміненим регістром літер на усі великі.   
...
У JavaScript усі числові дані, як цілочисельні значення, так і числа з  плаваючою комою представлені через тип даних (`number`). Для дуже великих цілих чисел (більше за модулем, ніж два в степені 53), застосовується спеціальний тип - `BigInt`.  
...
Цілочисельні літерали типів `Number` та `BigInt` можна виразити у різних формах. Це задається першою літерою літералу: 
...
Для запису значення типу `BigInt`, у кінці літералу добавляють символ `n`.  
...
Про об'єктні властивості та методи `Number` можна прочитати [за цим посиланням](https://wiki.developer.mozilla.org/uk/docs/Web/JavaScript/Guide/Numbers_and_dates) або [за цим](https://learn.javascript.ru/number).

### Стрічкові змінні (string)

Стрічка (`string`) в JavaScript повинна бути взята в лапки.

```javascript
let str = "Стрічка в подвійних лапках";
...
Крім звичайних символів до стрічок можна також включати спеціальні, як це показано в наступному прикладі. Весь перелік символів доступний [у розділі string з довідника](../Довідники/js/js.md#string).
...
Додатково про роботу з стрічковими змінними та виразами можна почитати за [цим посиланням](https://wiki.developer.mozilla.org/uk/docs/Web/JavaScript/Guide/Text_formatting) або [за цим](https://learn.javascript.ru/string).

### Перетворення типів 

Найчастіше оператори і функції автоматично перетворюють передані їм значення (примітивного типу) до потрібного типу. Наприклад, `console.log` автоматично перетворює будь-яке значення до стрічкового типу. Математичні оператори перетворюють значення до чисел. Але є випадки, коли потрібно явно перетворити значення в очікуваний тип.
...
value = String(value); 		// тепер це стрічка, що дорівнює "true"
console.log (typeof value); // string
```

#### В Number

Можна використати функцію `Number(value)`, щоб явно перетворити `value` в число. Якщо стрічка не може бути перетворена в число, результатом буде `NaN`.
...
Майже усі математичні оператори виконують чисельне перетворення, за виключенням  `+`. Якщо один із доданків є стрічкою, тоді всі інші приводяться до строчок і робиться конкатенація (з'єднання).   
...
У випадку, коли значення, що представляє число, знаходиться в пам'яті як стрічка, існують методи для перетворення [`parseInt()`](https://developer.mozilla.org/uk/docs/Web/JavaScript/Reference/Global_Objects/parseInt) та [`parseFloat()`](https://developer.mozilla.org/uk/docs/Web/JavaScript/Reference/parseFloat). 
...
Оператор порівняння порівнює операнди та повертає логічне значення істинності  порівняння. Операнди можуть бути числовими, стрічковими, логічними значеннями або об'єктами. Стрічки порівнюються згідно стандартного лексикографічного порядку, з використанням значень Unicode. 
...
На додачу до операторів порівняння, які можуть застосовуватись до  стрічкових значень, оператор конкатенації (+) об'єднує значення двох стрічок, повертаючи єдину стрічку. Наприклад,

```js
console.log('моя ' + 'стрічка'); // консоль виводить рядок "моя стрічка".
```

Для конкатенації стрічок може також застосовуватись скорочений оператор присвоєння +=  . Наприклад,
...
Як і в інших мовах, дозволяє робити галуження. 
...
Код з тіла циклу виконується, поки умова виконується. Наприклад, цикл нижче виводить `i`, поки `i < 3`       
...
#### Добавлення та видалення властивостей та методів
...
Можна використовувати числовий або стрічковий літерал для назви властивості, або вкладати об'єкт всередину іншого. У наступному прикладі використовуються ці можливості.
...
Окрім `for`, інструкції [`For..of`](https://wiki.developer.mozilla.org/uk/docs/Web/JavaScript/Reference/Statements/for...of) перебирають елементи в ітерабельних (перелічувальних) змінних (масивах, стрічках) .  
